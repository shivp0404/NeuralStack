...
exports.snoozeRevision = async (req, res) => {
  ...
  try {
    ...
    // Move all revision dates forward by N days
    const ms = days * 24 * 60 * 60 * 1000;
    ['day3', 'day7', 'day30'].forEach(key => {
      if (snippet.revisionSchedule[key]) {
        snippet.revisionSchedule[key] = new Date(new Date(snippet.revisionSchedule[key]).getTime() + ms);
      }
    });
    ...
    res.json({ message: `Snoozed all revisions by ${days} day(s)` });
  } catch (err) {
